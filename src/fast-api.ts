import type { FastConfig, FastTarget } from "./types.js";
...
export interface FastApiClient {
  getConfig(signal?: AbortSignal): Promise<FastConfig>;
}
...
export function createFastApiClient(options: FastApiClientOptions = {}): FastApiClient {
  ...
  return {
    async getConfig(signal?: AbortSignal): Promise<FastConfig> {
      ...
      try {
        ...
        try {
          config = (await response.json()) as FastConfig;
        } catch (error) {
...
function validateConfig(config: FastConfig): asserts config is FastConfig & { targets: FastTarget[] } {
...
export function formatLocation(location?: { city?: string; country?: string }): string | null {
  if (!location) {
    return null;
  }
